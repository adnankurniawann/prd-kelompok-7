...
export default function SpinPage() {
  ...
  const handleSpin = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(payload.error ?? "Spin gagal dijalankan.");
        return;
      }

      setResult(payload.data ?? null);
    } catch {
      setError("Tidak bisa menjangkau API spin.");
    } finally {
      setIsSpinning(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-5 py-6 sm:px-8 lg:px-10">
        ...
        <section className="mt-6 grid flex-1 gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="space-y-5 rounded-4xl border border-slate-200 bg-white/85 p-5 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur">
            <div className="rounded-3xl bg-slate-950 p-5 text-white">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl bg-white/10 p-4">
                  ...
                  <p className="mt-2 text-2xl font-black">Rp {budget.toLocaleString()}</p>
                </div>
                <div className="rounded-2xl bg-white/10 p-4">
                  <p className="text-[11px] uppercase tracking-[0.24em] text-slate-400">
                    Radius
            ...
            <div className="space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-5">
              <div>
                <label className="flex items-center justify-between text-sm font-semibold text-slate-800">
                  ...
                  <span className="text-rose-500">Rp {budget.toLocaleString()}</span>
                </label>
                <input
                  type="range"
                  min="10000"
            ...
            <div className="rounded-3xl border border-slate-200 bg-linear-to-br from-rose-500 via-orange-400 to-amber-300 p-5 text-white shadow-[0_20px_80px_rgba(244,63,94,0.22)]">
          ...
          <div className="space-y-5 rounded-4xl border border-slate-200 bg-slate-950 p-5 text-white shadow-[0_24px_100px_rgba(15,23,42,0.24)]">
            ...
            >
              <div className="absolute h-80 w-80 rounded-full border border-dashed border-white/15" />
              ...
              <motion.button
                ...
                onClick={handleSpin}
                disabled={isSpinning}
                className="relative z-10 flex h-44 w-44 items-center justify-center rounded-full border border-white/20 bg-linear-to-br from-rose-500 to-orange-400 text-2xl font-black shadow-[0_24px_70px_rgba(244,63,94,0.4)] transition-transform disabled:cursor-not-allowed"
              >
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4 text-sm text-slate-300">
              {error ? (
                <p className="rounded-2xl border border-rose-400/30 bg-rose-400/10 px-4 py-3 text-rose-100">
                  {error}
                </p>
              ) : (
                <p>
                  Backend akan memilih restoran terdekat yang aman dan masih masuk budget.
                </p>
              )}
            </div>
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold">
                    <span className="rounded-full bg-white/10 px-3 py-1">
                      Rp {result.price_tier.toLocaleString()}
                    </span>
                    <span className="rounded-full bg-white/10 px-3 py-1">
